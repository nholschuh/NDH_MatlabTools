...
% else
%     IC = zeros(1,max(length(a),length(b))-1);
...
if hasFilterX  % Use the faster C-mex filter function: -------------------------
    ...
    if max(IC) > 0
        ...
        [dum, Zi] = FilterX(b, a, Xi,Xi(1,:));   %#ok<ASGLU>
    end
    ...
    if max(IC) > 0
        ...
        [dum, Zf] = FilterX(b, a, Yf, Yf(nEdge, :), 'reverse');  %#ok<ASGLU>
    end
    ...
    if max(IC) > 0
        ...
        [dum, Zf] = filter(b, a, Yf, Yf(1, :));     %#ok<ASGLU>
    end
