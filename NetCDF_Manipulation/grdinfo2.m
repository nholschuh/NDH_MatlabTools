function d = grdinfo2(file,variable,timeslice,xvar,yvar);
% Adapted by Nick Holschuh - 2015
...
elseif (nvars==6),                        % old (v3) GMT netCDF grid file
...
elseif (nvars ~=3 & nvars ~=6)
    ...
    conv=[];
    desc=[]; zrange=[0 0]; vers='3.x format file';
    xrange=netcdf.getVar(ncid,xvar)';
    xname=netcdf.getAtt(ncid,xvar,'units');
    yrange=netcdf.getVar(ncid,yvar)';
    yname=netcdf.getAtt(ncid,yvar,'units');
    zrange=netcdf.getVar(ncid,variable)';
    ...
    if length(size(z)) == 3
        if exist('timeslice') == 0
            ...
            timeslice = inputdlg(promptentry);
            timeslice = str2double(timeslice);
        elseif timeslice == 0
            maxvalue = size(z);
            maxvalue = maxvalue(3);
            maxvalue = num2str(maxvalue);
