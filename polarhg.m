...
%%%%%%%%%%%%%%%%%%%%%%%%% Here, we decide if the plot is already a
%%%%%%%%%%%%%%%%%%%%%%%%% stereonet, and do not initialize the axes if it
%%%%%%%%%%%%%%%%%%%%%%%%% is.
...
if already_init == 0
    ...
    if ~HoldFlag
        ...
        patch([XX(1:end-1,4)],[YY(1:end-1,4)],'white')
        hhh = plot(XX(:),YY(:),':','Color',AxisColor,'LineWidth',1);
        plot(XX(:,4),YY(:,4),'Color','k','LineWidth',2)
...
%     xx = (Rmax-Rmin)*cos(rho).*cos(theta);
%     yy = (Rmax-Rmin)*cos(rho).*sin(theta);

% plot data on top of grid
q = plot(xx,yy,linestyle);
set(q,'Color',color,'LineWidth',2)
