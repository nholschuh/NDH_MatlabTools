...
keep_vars = {'d_lat','d_lon','d_elev','d_reflectUC','Quality','elev_use_flg'};
...
for i = 1:length(I.Attributes)
    wrt_str = ['output.GranuleMeta.',remove_illegalcharacters(I.Attributes(i).Name,' '),' = I.Attributes(i).Value;'];
    eval(wrt_str)
end
...
for i = 1:length(I.Groups)
    ...
    for j = 1:length(I.Groups(i).Groups)
        ...
        for k = 1:length(I.Groups(i).Groups(j).Attributes)
            wrt_str = ['output',rn3,'Meta.',remove_illegalcharacters(I.Groups(i).Groups(j).Attributes(k).Name,' '),' = I.Groups(i).Groups(j).Attributes(k).Value;'];
            eval(wrt_str)
        end
