function output = restructure_ATL11(data,filt_flag,out_form,addfieldpairs);
...
true_opts = {'pt1','pt2','pt3'};
atlas_sdp_gps_epoch = [1198800018.00000];
...
for i = 1:length(fs);
    ...
    if isfield(eval(['data.',fs{i}]),'h_corr')
        subdata = eval(['data.',fs{i}]);
        [x y] = polarstereo_fwd(subdata.latitude,subdata.longitude);
        h = subdata.h_corr;
        dt = subdata.delta_time;
        
        tval = datenum(1980,1,6,0,0,dt+atlas_sdp_gps_epoch);
        %     tval2 = datevec(tval);
        %     tval3 = datetime(tval2(:,1),tval2(:,2),tval2(:,3),tval2(:,4),tval2(:,5),tval2(:,6));
        
        
        if filt_flag == 1
            keeps = find(subdata.quality_summary == 1);
            xykeeps = find(min(subdata.quality_summary,[],1) == 1);
        else
            keeps = find(zeros(size(subdata.quality_summary)));
            xykeeps = find(zeros(length(subdata.quality_summary(1,:))));
        end
