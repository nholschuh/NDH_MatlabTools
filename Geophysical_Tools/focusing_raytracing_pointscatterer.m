function [true_midpoint_z true_p true_L_point transmit_angle reflect_angle true_time] = focusing_raytracing(srcz,offsets,travel_time,surf_rho,timerflag)
...
for i = 1:length(offsets)  
    ...
    true_L_point(:,i) = interp1(t_final(interp_inds,i),L_final_point(interp_inds,i),travel_time);
    true_L_plane(:,i) = interp1(t_final(interp_inds,i),L_final_plane(interp_inds,i),travel_time);
...
transmit_angle = ones(size(true_L_point))*NaN;
reflect_angle = ones(size(true_L_point))*NaN;
...
if debug_flag == 1
    hold off
    plot(true_midpoint_z(:,1),sqrt((1./true_L_plane(:,1))*(1/4/pi))/2,'Color','blue','LineWidth',2)
    hold all
    plot(true_midpoint_z(:,1),sqrt(sqrt((1./true_L_point(:,1))*(1/16/pi/pi))),'Color','red')
    plot_indicator_lines([1 0 0],3,'black',1,':')
end
