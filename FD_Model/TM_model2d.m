function [gather,tout,srcx,srcz,recx,recz] = TM_model2d(ep,mu,sig,xprop,zprop,srcloc,recloc,srcpulse,t,npml,outstep,plotopt,prefix)
% TM_model2d.m
% Plot Settings -
% 1 - Display Animation on the Spot
% 2 - Display Animation and write movie
% 3 - Write frames out as images without displaying them.
% 4 - 
% 5 - Save a Matrix with each step (while still displaying the animation)
% 
...
% %%%%%%%%%%%%%%%% NDH Added %%%%%%%%%%%%%%%%%%%%%%%%%%%
% [fspec faxis] = fft_ndh(srcpulse,t,2);
% center_f = find(max(fspec) == fspec);
% center_f = abs(faxis(center_f(1)));
% ep = ep-sqrt(-1)*sig/(2*pi*center_f*ep0);
...
asp_ratio = (max(zprop)-min(zprop))/(max(xprop)-min(xprop));
position_vec = [round(rand(1)*4)*500 800 560 round(560*asp_ratio)];
...
%%%%%%%%%%%%%%%%%%%%%%
%  Plot Initiation Factors
%%%%%%%%%%%%%%%%%%%%%%

if plotopt(1) == 1 | plotopt(1) == 2
    figure(1);
    set(gcf,'Position',position_vec,'Color','white');
end
if plotopt(1) == 3
    mkdir([prefix,'_Images'])
    delete(['./',prefix,'_Images/*'])
end
if plotopt(1) == 5
    ct5 = 1;
    %Ey_storage = zeros(nz-1,nx-1,numit);
end
...
for s=1:nsrc
    ...
    for it=1:numit
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % This is where an attenuation model can go (NDH 10/27/2015)
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
...
%         Ey = real(Ey);
        ...
        if plotopt(1)==1 | plotopt(1) == 5;   
            if mod(it-1,plotopt(2))==0
                ...
                if plotopt(1) == 5
                    Ey_storage(:,:,ct5) = Ey';
                    save([prefix,'_model_timelapse.mat'],'xEy','zEy','Ey_storage')
                    ct5 = ct5 + 1;
                end
            end
        elseif plotopt(1) == 2;
            if mod(it-1,plotopt(2))==0
                ...
                caxis([-plotopt(3) plotopt(3)]);
                pause(0.01);
                frames(counter1) = getframe(gcf);
                counter1 = counter1+1;
            end
        elseif plotopt(1) == 3;
            if mod(it-1,plotopt(2))==0
                savename = ['./',prefix,'_Images/FDTD_Source_',sprintf('%03d',s),'_Frame_',sprintf('%04d',counter1),'.jpg'];
                ttt = colormap(gray);
                imwrite(get_index_from_colormap(Ey',[-plotopt(3) plotopt(3)],ttt),ttt,savename);
                counter1 = counter1 + 1;
            end
        end
        ...
        if mod(it,500) == 0
            disp(['Completed Time Step ',num2str(it),' of ',num2str(numit),' - ',num2str(round(toc)),'s']);

        end
    end
    
    if plotopt(1) == 2
        vid_ob = VideoWriter([prefix,'_',date,'_FDTD_Src_',num2str(s),'.avi'],'Motion JPEG AVI');
        vid_ob.FrameRate = 15;
        vid_ob.Quality = 75;
