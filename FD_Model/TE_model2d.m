function [gather_x,gather_z,tout,srcx,srcz,recx,recz] = TE_model2d(ep,mu,sig,xprop,zprop,srcloc,recloc,srcpulse,t,npml,outstep,plotopt,prefix)
% TE_model2d.m
% 
    ...
    for it=1:numit
        ...
        if plotopt(1)==1 | plotopt(1) == 5;   
            if mod(it-1,plotopt(3))==0
                ...
                if plotopt(1) == 5
                    Ex_storage(:,:,ct5) = Ex';
                    Ez_storage(:,:,ct5) = Ez';
                    save([prefix,'_model_timelapse.mat'],'xEz','zEz','Ez_storage','xEx','zEx','Ex_storage')
                    ct5 = ct5 + 1;
                end
            end
        elseif plotopt(1) == 2;
        ...
        elseif plotopt(1) == 3;
            if mod(it-1,plotopt(3))==0
                ...
                ttt = colormap(gray);
                if plotopt(2) == 1
                    imwrite(get_index_from_colormap(Ex',[-plotopt(4) plotopt(4)],ttt),ttt,savename);
                elseif plotopt(2) == 2
                    imwrite(get_index_from_colormap(Ez',[-plotopt(4) plotopt(4)],ttt),ttt,savename);
                end
                counter1 = counter1 + 1;
            end
        end
        ...
        if mod(it-1,outstep)==0
            ...
            for r=1:nrec
                gather_x((it-1)/outstep+1,r,s) = Ex(reci(r),recj(r));
                gather_z((it-1)/outstep+1,r,s) = Ez(reci(r),recj(r));
            end
        end
